      ...
      def self.included(base)
        base.class_eval do
          ...
          def invalidate_wikiscripts_key
            if (ancestors.map(&:title) + [title, title_was]).include?(Setting["plugin_redmine_scripting_engine"]["wikiscripts_root"])
...
unless WikiPage.included_modules.include?(RedmineScriptingEngine::Patches::WikiPagePatch)
  WikiPage.send(:include, RedmineScriptingEngine::Patches::WikiPagePatch)
end
